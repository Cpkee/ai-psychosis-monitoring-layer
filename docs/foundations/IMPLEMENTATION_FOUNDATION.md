...
Test counts today: **446 total** — 45 governance, seal and seal-screen tests that run on a bare interpreter; the rest are repository, contract, validator, runner, trajectory, alert, seed-pipeline, database-guard and HTTP-retry tests. 85 skip without `TEST_DATABASE_URL` (84 PostgreSQL, plus the opt-in live extractor test).

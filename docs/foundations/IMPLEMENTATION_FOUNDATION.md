...
Test counts today: **543 total** — 60 governance, seal, seal-screen and overlap-screen tests that run on a bare interpreter; the rest are repository, contract, validator, runner, trajectory, alert, seed-pipeline, database-guard and HTTP-retry tests. 111 skip without `TEST_DATABASE_URL` (110 PostgreSQL, plus the opt-in live extractor test).
...
| S0–S8 | **Seed pipeline** — S0 ✅, S1 ✅, S2 ✅, S3 ✅, S4 next | Collect → Filter → Choose → Generate → Review → Label → Adjudicate, on a shared PostgreSQL. Full plan: [`SEED_PIPELINE_PLAN.md`](SEED_PIPELINE_PLAN.md). Outside the tracer bullet (§7); supplies the corpus for the pilot and for increment 8. | — |
...
**S3 decisions (D-49).** Taken while building S3 Choose, 2026-09-25.

| ID | Decision | Why |
|---|---|---|
| **D-49** | **S3 Choose as built.** (a) A **selection** is an immutable snapshot of every chosen seed, versioned 1, 2, 3 …, each superseding the one before; storage keeps one unbroken chain. (b) A seed's **split is assigned once, in its own table**, and never changes: a seed dropped from a later selection keeps it, and every selection entry must agree with it (foreign key). (c) Only S2-eligible seeds (clear, or kept) from a document's current extraction can be chosen. A chosen seed that later becomes ineligible **blocks the next selection until the chooser drops it explicitly**; nothing is removed silently. (d) The coverage matrix is theme family × explicitness × **harm described or not**, since harm types are free text. Each cell shows seeds per split, eligible seeds left unchosen, and the chosen seeds' account kinds and tiers. The snapshot stores the unchosen pool too, so a gap shows whether supply or choice caused it. (e) The **benign-hard-negative dimension is absent**: benign counterparts are derived from chosen seeds at S4 (D-36), which adds it. (f) Targets (`coverage_targets_v0.1`) are advisory and provisional (OD-027), with a fingerprinted version. A selection that misses one is refused until `--accept-gaps`, and every accepted gap is stored on it in plain words. (g) **Exposure log:** anyone shown a seed's content by `choose` or by S2's `review-flags`, or who chooses it, is recorded once per seed and activity; showing content requires `APML_ACTOR_ID`. S6 adds `review` and `revise`, and must extend the table's `CHECK`. (h) No pilot marker at S3: the S5 simulated-user pilot takes its 5 seeds from the **development** split, which is how "pilot seeds are always development" holds. (i) Narrower than planned, as D-48h: selections go to whichever database `--database` selects. | (b) A split in each selection could not stop a seed moving between splits across versions; one row per seed lets storage enforce it (D-6). (c) Silently dropping a seed would change a split's membership without anyone deciding it. (e) Absent beats stubbed: nothing exists yet to count. (g) Exposure cannot be reconstructed afterwards from who merely *looked*, and S7 blind assignment depends on it. (h) Avoids a marker with no consumer until S5. |

**Labelling decision (D-50).** Taken with the project lead on 2026-09-25, while planning S7 and S8.

| ID | Decision | Why |
|---|---|---|
| **D-50** | **Labelling works on files.** Annotation packets (S7) and adjudication packets (S8) are **spreadsheet files**, one per person per batch. Every packet comes back through **one validated import**. It applies the rules judge output must follow (scale from config, `null` never `0`, evidence turns for any score above 0), and refuses a packet whose checksum does not match the turns it was built from. An optional **local** annotation UI opens and saves the same file. It is not hosted, needs no sign-in and has no database connection ([OD-009](OPEN_DECISIONS.md#od-009)). Adjudication, disagreement compilation and reference-set release, defined in `architecture.md` §6.12 but assigned to no increment, become **S8**. The spreadsheet format needs a library (for example `openpyxl`); it stays out of the governance suite and is confirmed when S7 is built. | The proposed adjudicator is an external clinical doctor, who cannot be given database access ([OD-005](OPEN_DECISIONS.md#od-005)). One import path makes labels from a spreadsheet and from the UI indistinguishable, which agreement statistics need. A hosted UI would need real authentication and would put harm-bearing content on a server ([OD-026](OPEN_DECISIONS.md#od-026)). Without S8, S7 would yield two sets of independent labels and no reference standard. |

**Corpus tuning (D-51).** Taken with the project lead on 2026-09-25, after the first automated collection run.

| ID | Decision | Why |
|---|---|---|
| **D-51** | **Only in-scope accounts become seeds, and the searches are narrowed.** `extraction_prompt_v0.4` admits an account only if a belief grew through the conversations, the person came to rely on or become attached to the AI, or the conversations were connected with harm. Ordinary use (information, advice, tasks, study, health questions), attitudes towards AI, tools used by professionals, and evaluations of AI systems yield an empty list. The prompt also asks for people and settings to be described without clinical roles or conditions. `queries_v0.3` replaces `psychos*` with `psychosis OR psychotic`, and a bare `attachment` with emotional attachment, dependence or reliance and parasocial. Finished documents are re-extracted with `reextract --stale`; earlier seeds stay stored as history (D-45). | In the first run, 15 documents gave 3 off-topic seeds with no theme (cancer survivors, breast-cancer screening, whiplash recovery) that would have passed S2 and reached `choose`. 3 more failed validation because the extractor described off-topic clinical settings with diagnostic terms. Both came from the prompt treating any use of an AI as an account. `psychos*` also matched "psychosocial" and "psychosomatic", so most PubMed results were off-topic. |

| **D-52** | **The prompt names the words the validator refuses, and `choose` shows each seed's source.** `extraction_prompt_v0.5` injects the prohibited terms from `seed_vocabulary_v0.2.json` through a `{prohibited_terms}` placeholder, with the rule that a longer word containing one also counts. The list stays in one place, and the vocabulary is already part of the prompt version. `seeds.py choose` lists each candidate's source title, identifier (PMID, DOI or arXiv id) and link. | Under v0.4, a case report of an A1 individual (a chatbot echoing a man's sense of spiritual awakening), the kind of seed the corpus lacks, failed validation. The extractor wrote "manic", and "antipsychotic medication", which contains "psychotic". The prompt said "no diagnostic words" without saying which. A chooser needs the source to judge credibility, especially for gold. |

| **D-53** | **Refused words cover reported speech; A1 gets its own searches; re-extraction is targeted.** `extraction_prompt_v0.6` extends D-52's rule to words the AI or other people are reported as saying. The one document that failed under v0.5 is re-run alone (`reextract --document`), **not** the whole corpus, so the corpus is deliberately mixed: most documents stay on v0.5, and the case report and anything collected later use v0.6. Each document records its own version (D-45). `reextract --stale` now lists every v0.5 document; run it only to regenerate the corpus on purpose. `queries_v0.4` adds a PubMed and an arXiv search for theme family A1, with terms from its definition (spiritual, religious, messianic, grandiose, awakening, destiny, mystical), never from Psychosis-Bench. | After v0.5, the corpus had **no A1 seed**. The only A1 individual, a case report of a chatbot echoing a man's sense of spiritual awakening, failed because it reported the chatbot downplaying "a manic episode". Re-running all 41 documents would have reshuffled seeds again: between v0.4 and v0.5 the APA article's A1 pattern seed disappeared. |

| **D-54** | **A person can exclude an off-topic seed, and the A1 searches are withdrawn.** `seeds.py exclude --seed ID --reason TEXT` records an attributed, append-only relevance decision; `--undo` supersedes it with an inclusion, and the exclusion stays on record. An excluded seed leaves the `choose` candidates and the coverage counts. If it is already in the selection, the next selection must drop it explicitly, as for any seed that stops being eligible (D-49c). Excluding a seed records a `choose` exposure. Storage keeps one unbroken chain of decisions per seed, and an inclusion can only undo an exclusion. `queries_v0.5` removes the two A1 searches added by D-53. | The extractor still turns a few off-topic documents into seeds (8 of 46: a blockchain paper labelled A1, a survey on HIV conspiracy beliefs, an opinion piece), and nothing but a person's memory kept them out of `choose`. The original plan listed relevance as a human step (D-37); this restores it without touching S2, whose screen is about benchmark overlap, not topic. The A1 searches returned 39 documents, no A1 account and 5 off-topic seeds: academic literature reports almost no spiritual or messianic accounts, so A1 must come from journalism and case reports added by hand. |

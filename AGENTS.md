...
Without `DATABASE_URL` the PostgreSQL tests skip and everything else runs. **411 tests**; 83 skip without a database (82 PostgreSQL, plus the opt-in live extractor test: `APML_LIVE_TESTS=1`).
...
| S2–S7 | **⬅ next: S2 Filter.** Seed pipeline: Filter → Choose → Generate → Review → Label. Plan: `docs/foundations/SEED_PIPELINE_PLAN.md`, decisions D-23…D-40 | Unblocked | S5 generation: [OD-022](docs/foundations/OPEN_DECISIONS.md) simulated-user pilot. S1 shared writes: OD-024, OD-026 |
...
**One finding that needs a decision, not just a note:** current Codex models reject `temperature`/`top_p`/`top_k` with a 400, so [`PROJECT_SCOPE.md`](docs/foundations/PROJECT_SCOPE.md) **NFR-1 ("any stored score can be re-derived") cannot be met as written** and must be amended. See `JUDGE_DECISIONS.md` §2.
...
- `extraction_v0.1` names `gemini-2.5-flash`; confirm it is an exact pinned version before the first official run (it is part of the cache key).

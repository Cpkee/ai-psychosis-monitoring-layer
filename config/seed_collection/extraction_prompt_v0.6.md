<!--
  Seed extraction prompt, version extraction_prompt_v0.6
  ...
  v0.5 (D-52): the words the validator refuses are listed in the prompt, injected
  from seed_vocabulary_v0.2.json, with the rule that a longer word containing one
  also counts ("antipsychotic" contains "psychotic").
  v0.6 (D-53): the rule covers words the AI or other people are reported as
  saying, not only the extractor's own framing.
  ...
  Placeholders: {theme_lines} {phase_lines} {explicitness_values}
                {behaviour_lines} {account_kind_lines} {prohibited_terms}
                {document}
-->
...
In `arc_summary` and `harm_type_candidate`, never use any of these, **even inside
a longer word**, and **even when reporting what the AI or other people said**:
{prohibited_terms}. For example, write "stopped taking prescribed
medication", not "stopped taking antipsychotic medication", because
"antipsychotic" contains "psychotic"; write "weeks of little sleep and racing
speech", not a clinical label for it; and write "the chatbot downplayed the idea
that anything was wrong", not the clinical term the chatbot or a clinician used. Only `raw_theme_terms` may quote the
